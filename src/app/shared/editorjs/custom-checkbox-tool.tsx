...
import CheckboxOptionsComponent from "./components/CheckBox";
...
class CheckBoxTool implements BlockTool {
  private data: { options: CheckboxOption[] };
  private onChange: (data: { options: CheckboxOption[] }) => void;
  ...
  constructor({
    ...
    this.data = data;
    this.onChange = (newData: { options: CheckboxOption[] }) => {
  ...
  validate(savedData: CheckboxOptionsData) {
    // Allow saving if there are options and at least one has text
    if (!savedData.options || savedData.options.length === 0) {
      return false;
    }
    return savedData.options.some((option) => option.text.trim() !== "");
  }
